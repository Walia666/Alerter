...
def applogalert(request):
   request.session['Username'] = "Anshul"
   request.session['emp_id']= "C2445"
      ...
      if request.method == 'POST':
       ...
       #  the following is the user_dn format provided by the ldap server
        ...
        try:
                ...
                return redirect('/application/applogalert/')
                connect.unbind_s()
...
def docsnew(request):
  ...
  if request.method == 'POST':
  	...
  	portint=int(portmain)
        ...
        try: 
  	  client=Elasticsearch(host=hostmain, port=portint,max_retries=10, retry_on_timeout=True)
  ...
  "docvalue_fields": [
    "@timestamp"
  	  ...
  	  result= client.search(index=index, body=document,scroll='2m',ignore=504)
     			...
     			keyarray1.extend(keyarray3)
      			...
      			with open("fields.csv", "wb") as myfile:
        			...
        			for row in splitlist:
                                        row=[item.encode('utf-8') for item in row if item != '' ]
          				writer.writerow(row)
      				stat="true"
      			if stat == "true":
...
def alertinfo(request):
		...
		obje=Alert.objects.filter(cluster="systemlog")
			...
			alert_name=row["_source"]["alert_name"]
                ...
                for name in alert_nameunique:
                	body={"sort" : [{ "lastalertsent" : {"order" : "asc"}}], "query": { "match": { "alert_name": name} }}
		...
		for size in sizelist:
                ...
                return render(request, 'application/alertinfo.html', {'kibanaquery':zippedlist,'alert_name':time})
